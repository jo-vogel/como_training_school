...
setwd("C:/Users/vogel/boxup/Promotion/Damocles COST Action/Training school/Group_project/Data")
# read in HR dataset 
# data <- fread( list.files( "data", full.names = TRUE )[2] ) # apparently not the right one
data <- fread( list.files( "data", full.names = TRUE )[1] ) # this seems to be the right one
str(data)
...
# list( model_glm_sum$coefficient, 1- ( model_glm_sum$deviance / model_glm_sum$null.deviance ) )
...
# source("logistic_regression_code/logistic_functions.R") # seems to be wrong name
setwd("C:/Users/vogel/boxup/Promotion/Damocles COST Action/Training school/Group_project/Code")
source("unbalanced_funtions.R") # right name
# browser()
