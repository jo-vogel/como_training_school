...
#Pauline's Laptop
load("C:/Users/admin/Documents/Damocles_training_school_Como/GroupProject1/Data/Global/extremeindices_and_monthlymeteovar_rescaled_mean_995pix.Rdata")

pix_num <- length(Data_xtrm_standardized_mean$longitudes)
nb_years <- dim(Data_xtrm_standardized_mean$yield)[2]

##### Process data #####
yield_3dim <- array(Data_xtrm_standardized_mean$yield,dim=c(pix_num,1,nb_years))
dtr_3dim <- array(Data_xtrm_standardized_mean$dtr,dim=c(pix_num,1,nb_years))
frs_3dim <- array(Data_xtrm_standardized_mean$frs,dim=c(pix_num,1,nb_years))
txx_3dim <- array(Data_xtrm_standardized_mean$txx,dim=c(pix_num,1,nb_years))
tnn_3dim <- array(Data_xtrm_standardized_mean$tnn,dim=c(pix_num,1,nb_years))
rx5_3dim <- array(Data_xtrm_standardized_mean$rx5,dim=c(pix_num,1,nb_years))
tx90p_3dim <- array(Data_xtrm_standardized_mean$tx90p,dim=c(pix_num,1,nb_years))
tn10p_3dim <- array(Data_xtrm_standardized_mean$tn10p,dim=c(pix_num,1,nb_years))

Model_data <- abind(yield_3dim,dtr_3dim,frs_3dim,txx_3dim,tnn_3dim,rx5_3dim,tx90p_3dim,tn10p_3dim
                    ,Data_xtrm_standardized_mean$tasmax,Data_xtrm_standardized_mean$vpd,Data_xtrm_standardized_mean$pr,along=2)
...
Yield <- Data_xtrm_standardized_mean$yield
...
save(model_cv_fitting, file = paste0("C:/Users/admin/Documents/Damocles_training_school_Como/GroupProject1/RidgeRegression/Global_results/cv_month_xtrm_LASSO_threshbadyield005_seed",
                                     seed, "_train", train_size,"_995pixels_meanstand.RData"))
